...
class NominationAdmin(admin.ModelAdmin):
    list_display = [
        'photo_thumbnail', 'nominee_name', 'get_category_name',
        'get_group_name', 'nominator_name', 'nominator_phone',
        'nominator_email', 'created_at'
    ]
    list_filter = ['category__group_name', 'category', 'created_at']
    search_fields = ['nominee_name', 'nominator_name', 'nominator_email', 'nominator_phone']
    readonly_fields = ['photo_preview', 'nominee_name_normalized', 'created_at']
    actions = ['export_nominations_csv']

    fieldsets = (
        ("Nominee Information", {
            "fields": ("nominee_name", "category", "nominee_photo", "photo_preview")
        }),
    ...
    def export_nominations_csv(self, request, queryset):
        ...
        writer.writerow([
            'Nominee Name', 'Category Group', 'Award Category', 'Photo URL',
